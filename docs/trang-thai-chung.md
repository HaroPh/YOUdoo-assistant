...
| 27 | ✅ **Retriever ta trượt câu bảng-nhỏ-trong-thuyết-minh — ĐÓNG** (`467f33a`, push 2026-09-19). CÒN: corpus sản xuất KHÔNG tự nhận vá (`_hash` chỉ băm byte tệp) — 544 chunk `public` giữ breadcrumb rác tới khi xoá `rag_documents` + nạp lại 18 tài liệu (~15 phút, VLM gọi thật ~16+ lượt, nay sổ đếm được) | chờ chủ dự án đồng ý nạp lại | ghi chú thi hành |
| 31 | **Lát 5 (trang thuyết minh) MERGED `20d58d5`, push 2026-09-19** — nghiệm thu mới 1 tài liệu/5 trang. CÒN: chạy 4 PDF scan còn lại + SID (~2.500 token/trang); `loai=hieu` chưa có tầng kiểm; trần ~49% trang | chưa ai | ghi chú thi hành, mục "Lát 5" |
