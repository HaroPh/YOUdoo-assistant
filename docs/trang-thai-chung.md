...
| 34 | ⚠️ **Tệp đính kèm không cho nguồn → backend LẶNG LẼ trả lời từ corpus chung**, trích dẫn trông chuẩn nhưng SAI TÀI LIỆU (đo 2026-09-19: hỏi NTC, nhận số của SID). Cần: khi có tệp đính kèm mà Open WebUI gửi 0 nguồn từ nó, trả lời phải nói rõ nguồn là kho chung | chưa ai | ghi chú thi hành, mục "NGHIỆM THU ĐẦU-CUỐI" |
| 35 | **`_CHUONG_RE = ^\s*Chương` khớp cả "Chương TRÌNH"** → "Chương trình phần mềm máy tính" thành gốc cấp 1 của cả SID, mọi trích dẫn SID dẫn đầu bằng nó. Sửa: đòi số/La Mã sau `Chương` | chưa ai | `src/rag/parse.py` |
