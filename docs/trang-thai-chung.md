...
| 19b | ✅ **ĐÓNG** (2026-09-20, 10 task TDD): vai `warehouse` không còn đọc được 4 tài liệu thương mại qua RAG; `accounting`/`sales`/`admin` vẫn đọc được. Nhãn là **lớp** (`all`|`commercial`) ở `rag_chunks.visibility`; `retrieve()` lọc bằng SQL trên **cả ba** chân dense/sparse/fold **trước** `LIMIT`, fail-closed (`None`/rỗng → `{'all'}`, chỉ sentinel `UNRESTRICTED` mới mở). Migration 009 đã chạy trên DB thật: `[('all',3877),('commercial',24)]`, gỡ 1 tài liệu SID / 660 chunk (17 tài liệu / 3 901 chunk). **Nghiệm thu:** cổng DƯƠNG `GATE PASS 0.963=0.963` (admin r@20 0,9771 = mốc cũ); cổng ÂM `thương mại 10 ca lộ 0, khác 99 ca kém đi 0`; probe sống 4 vai — kho hỏi chính sách chiết khấu nhận câu trả lời về luật doanh nghiệp, 3 vai kia đọc đúng. Phần sụt của kho 0,0918 ≈ 10/109 = đúng 10 ca thương mại, không ca nào khác mất. Ảnh trước khi chạy: schema `backup_pre009` | xong | spec §10 có đủ số đo + 5 giới hạn còn lại |
...
| 28 | ✅ **ĐÓNG** (`4dcdc65`, merge `51ce928`, 2026-09-19): `_gate` có nhánh `retrieval` — r@20 không dung sai (trần pool), r@6 dung sai 1/n, mrr không gác, từ chối baseline khác `dang_go`/`rerank`. Hoá ra là **hai** KeyError chồng nhau (`false_confirm` rồi `acc` ở dòng in GATE). Nghiệm thu thật: GATE PASS, r@20 0,9771 · r@6 0,9633 = baseline — **mốc TRƯỚC 19b**. **ĐÃ wire vào job 2026-09-24**: `retrieval` nay nằm trong `EVAL_FN` + `--set all`, neo baseline theo model NHÚNG (`bge-m3`) chứ không theo `BASELINE_MODEL` chat, và `n` thành điều kiện hợp lệ thứ ba bên cạnh `dang_go`/`rerank`. Chạy thật PASS (r@6 0,963, r@20 0,9771), đột biến baseline → FAIL exit 1. Lịch chạy vẫn là việc của #25 | xong | `tests/jobs/test_retrieval_gate.py` |
| 29 | ✅ **Sổ `llm_usage` đếm được lượt VLM** — ĐÓNG 2026-09-19 (`2e75d51` + `0c9a122`). Vòng 1 chỉ đặt sổ ở nhà máy nên bị đi vòng qua ngay trong ngày; vòng 2 đưa `_SoVlm` thành MẶC ĐỊNH của `VisionReader`. Rào `so_vlm_khong_cham_postgres` chặn test ghi vào sổ thật | xong | còn **18 dòng rác `p=1200 c=300 t=1500`** trong `public.llm_usage` do suite ghi trước khi có rào — chờ chủ dự án quyết xoá |
| 30 | ✅ **ĐÓNG** (2026-09-20, chẩn đoán, không sửa mã): đường ghi sổ **không hỏng** — 5 lượt chat probe 19b qua `:8002` lúc 14:15 → 15 dòng (3 lời gọi LLM/lượt), 19/09 có 126 dòng. Khoảng trống 13→18/09 là vì chat những ngày đó **không đi qua backend Youdoo**: chủ dự án làm `D:\starter` (:3000) và một assignment trên laptop khác; `.env` Youdoo sửa lần cuối 11/09 vẫn trỏ 5434; container `postgres` của `D:\Project` (5433, cùng tên DB) sống tới ~17/09 rồi dừng. **Lưu ý cho #25/#15:** trong khoảng đó API key dùng chung 3 nơi ⇒ sổ chỉ là **cận dưới** (giải thích một phần vụ 179 ghi sổ vs 500 Google đếm). Chủ dự án xác nhận **không còn dùng các code đó** ⇒ từ 20/09 sổ lại phản ánh đủ; hễ sau này có tiêu dùng ngoài repo thì phải nhớ sổ là cận dưới | xong | `public.llm_usage`; chưa xác nhận bằng số trên 5433 vì container đã dừng — muốn chắc thì bật lại và đếm 13–18 |
...
| 34 | ✅ **ĐÓNG dạng thu gọn** (2026-09-19): footer `📄 Nguồn: kho tài liệu chung` nêu LỚP nguồn. KHÔNG phát hiện được tệp đính kèm ở phía ta — Open WebUI pop cả `files` (middleware.py:2600) lẫn `metadata` (openai.py:1207) trước khi gọi, request có-tệp-0-nguồn và không-tệp giống nhau từng byte. Muốn hơn phải sửa phía Open WebUI (inlet filter) | xong | `synthesis.build_citations` |
| 35 | ✅ **ĐÓNG** (`2686310`, merge `c19849f`): `_CHUONG_RE` đòi số/La Mã. Đo trước: 2 lá / 337 chunk bắt nhầm, `Mục` 0 nhầm nên không đụng. Corpus sản xuất CHƯA nạp lại sau sửa — 336 chunk SID còn gốc "Chương trình phần mềm máy tính". **⚠️ ĐỪNG nạp lại SID (2026-09-19):** 19b đã chốt GỠ tệp SID khỏi corpus chung — 336 chunk lỗi biến mất theo tệp, tiết kiệm 25 lượt VLM ~70k token. Fix sẽ có tác dụng tự nhiên ở lần nạp `seed/` kế | ✅ xong — 19b đã gỡ tệp SID khỏi corpus (2026-09-20), 336 chunk lỗi biến mất theo tệp, không tốn lượt VLM nào | `src/rag/parse.py` |
| 36 | ✅ **CI đỏ từ 04/09 — ĐÓNG** (`e13c246`): một test gọi `read_page` không mock `osd_rotation` (thêm 11/09) → `TesseractMissing` trên runner không có Tesseract; local xanh vì máy dev có. Run `c19849f` xanh lần đầu sau 15 ngày | xong | `tests/ocr/test_document.py` |
| 37 | **Benchmark ngoài có qrels thật (2026-09-23)** — TVPL-Retrieval-VN (1.000 câu) + Zalo AI Legal Text Retrieval (788 câu, corpus 61k Điều), nạp qua đúng bộ cắt/biến đổi ingest vào schema riêng, mọi chân chung một pool; 0 lỗi, qua cổng R12. **(a) Qwen3-Reranker-0.6B THUA bge-reranker-v2-m3** cùng chế độ ở cả hai bộ (MRR −0,012…−0,060, p ≤ 0,03), chậm ~3,5× ⇒ đảo kết luận "0.6B-override hơn bge" của #31: phần lợi đó là của chế độ override, không của model. **(b) `override` > `blend` với bge**: MRR +0,038/+0,046, p < 0,0001 trên CẢ HAI bộ; recall@6 không tụt (+0,012 p=0,023 / +0,0025 ns) nhưng có câu văng (9/5) — **chủ dự án quyết có bật `RAG_RERANK_MODE=override` cho production không**. (c) Mất lớn nhất nằm trước reranker: 7–8,5% đáp án không vào pool 20. **(d) Stress test vai (nhãn `commercial` GIẢ LẬP, mật độ 27–40% chunk):** 0 rò rỉ; accounting trùng admin từng câu; tín hiệu "bị chặn" bắt 96–98%; nhưng luật top-3 **từ chối oan 40,6% / 43,4%** câu mà kho ĐÃ tìm đúng đáp án — tỉ lệ oan tăng theo mật độ nội dung bị giấu, nên con số 0/990 của 19c sẽ không giữ khi 19b mở lại với nhiều tài liệu nội bộ. **(e) Thứ hạng không tất định**: chân FTS `ORDER BY score` thiếu khoá phá hoà (117/200 câu hoà `ts_rank`), `UPDATE` bảng đổi thứ tự top-6 ở 2,1% câu — sửa rẻ `, c.id`, CHƯA sửa. (f) Ingest production ghi từng dòng 46 ms/dòng; `executemany` nhanh ~15× — CHƯA sửa. 4B chưa đo trên bộ ngoài | chủ dự án (b); chưa ai (d)(e)(f) | `backend/evals/results/bench-ngoai-2026-09-23/README.md` |
| 19c | ✅ **ĐÓNG** (2026-09-22, 10 task TDD, nối tiếp 19b): vai bị chặn (`warehouse`) nay nhận **câu từ chối tất định nêu đúng phòng ban** thay vì câu trả lời tự tin lạc đề, trên CẢ hai tuyến `rag` và `mixed`. Cơ chế: `retrieve()` chạy thêm một truy vấn bóng KHÔNG lọc vai (chỉ khi vai đang bị hạn chế), lấy **top-3** ứng viên của bản bóng (`HIDDEN_TOP_K`, hằng cạnh `VIS_IDX`) — đổi từ luật hạng-1 ban đầu sau khi đo thật (xem dưới); nếu lớp bị giấu nằm trong đó thì `rag_node`/`gather_docs` trả `denied_message()` tất định (không LLM), `fuse_answer` nối câu đó vào cuối câu trả lời ERP trên tuyến mixed. **Số đo cuối (một lượt, câu có dấu):** cổng ÂM `thương mại 10 ca (lộ 0, không báo chặn 1 — đã biết được miễn), khác 99 ca (kém đi 0, từ chối oan 0)`; cổng DƯƠNG admin `0,963=0,963` không hồi quy; lat_p50 admin 523ms / kho 558ms (+35ms trên corpus thật ~3 900 chunk — KHÁC con số +4,2ms đo trên fixture 2 tài liệu ở Task 5, con số đó không dùng được cho corpus thật); probe sống PASS cả `rag` và `mixed` trên hạ tầng thật, đối chứng vai `sales` thấy đủ trích dẫn. **Giả thuyết ban đầu bị bác bởi số đo:** plan kỳ vọng luật hạng-1 bắt 10/10 câu thương mại; đo thật chỉ 5/10 (0 lộ, 0 từ chối oan, một lượt câu có dấu) — bảng theo k (1→5: 5/10, 6/10, 9/10, 9/10, 9/10) cho thấy top-3 là điểm tốt nhất (9/10, vẫn 0/99 oan; k=5 mới sinh 1/99 oan). Chủ dự án chọn top-3 từ bảng đó — đây là đánh đổi sản phẩm, không phải quyết định kỹ thuật thuần. **Ba việc còn mở (tại thời điểm đóng 10 task):** (1) trên tuyến mixed, model fusion TỰ VIẾT một đoạn lý do SAI (đổ lỗi "ERP không có dữ liệu") đứng TRƯỚC câu từ chối tất định đúng — máy không bắt được vì đoạn đó không chứa marker, chỉ thấy khi đọc nguyên văn bằng mắt; sửa cần A/B prompt fuse, ngoài phạm vi nhánh này; (2) một câu thương mại ("đóng gói hàng ra sao trước khi chuyển đi", tài liệu `sla.docx`) đứng hạng > 5 trong bản bóng — miễn qua `KNOWN_UNFLAGGED` có kiểm rữa staleness (cổng FAIL nếu ca này bắt đầu bị bắt hoặc câu hỏi biến mất khỏi bộ ca); (3) tuyến thật của câu mixed trong probe sống KHÔNG được xác nhận độc lập qua Langfuse/log — bằng chứng chỉ là câu trả lời có cả ERP thật lẫn câu từ chối, nhất quán với mixed nhưng chưa soi trace. **Sóng sửa cuối sau review toàn nhánh (2026-09-22, ruling controller — xem spec §10 Task 11):** ba mục phải sửa trước merge. **C1 (Critical):** lượt bóng cũ hợp nhất câu hiện tại với lượt người dùng TRƯỚC (aux) ngang trọng số RRF — đo qua `measure_hidden_multiturn.py` (990+990+400 cặp câu hỏi thật, seed 20260922): sau câu thương mại, **32,0%** câu tiếp theo KHÔNG liên quan bị TỪ CHỐI OAN; khi câu trước không liên quan, **67,0%** câu thương mại bị BỎ SÓT (chỉ bắt 33,0%). Sửa: lượt bóng chỉ dùng câu hiện tại (`prepared[:1]`) — cho **0/990** từ chối oan và **90,0%** bắt đúng, tốt hơn trên CẢ HAI trục, không phải đánh đổi. Lượt LỌC không đổi (vẫn dùng cả aux). **I1:** câu "không k nào bắt được" (kể cả top-20) cho ca `sla.docx` ở trên là SAI — do chính controller viết ra rồi lan ra 4 chỗ trong repo. Đo lại trực tiếp hạng của chunk bị giấu trong bản bóng (đo gốc: 35 ứng viên; tái lập được bằng `measure_hidden_topk.py` sau khi nâng `MAX_K=6`): nằm ở **HẠNG 6**, tức k=6 BẮT ĐƯỢC. Câu đúng: bắt được cần **k ≥ 6**, nhưng từ chối oan đã xuất hiện từ k=5 (1/99) — lý do miễn là đánh đổi k, không phải giới hạn truy xuất. **I2 (đo, không sửa mã):** chạy lại cổng ÂM với `--dang-go khong_dau` — thương mại 10 ca (lộ 0, **không báo chặn 8** — 1 ca đã biết được miễn, 7 ca mới), khác 99 ca (từ chối oan 0). Tức 0 lộ, 0 từ chối oan vẫn giữ, nhưng chỉ **2/10** được báo (so 9/10 khi có dấu) — sai về phía BỎ SÓT, đúng phía an toàn; ghi nhận, KHÔNG sửa mã (cổng chính thức vẫn là dạng có dấu). **Chưa đo (nêu rõ, không phỏng đoán):** câu thương mại chỉ "trông giống" thương mại NHỜ ngữ cảnh lượt trước (ví dụ câu tỉnh lược kiểu "trong bao lâu?" tiếp sau câu về SLA) — sau khi C1 sửa, lượt bóng không còn thấy ngữ cảnh đó nữa, và độ ảnh hưởng lên NHÓM câu hỏi này chưa được đo (`measure_hidden_multiturn.py` chỉ đo các cặp câu ĐỘC LẬP, không đo câu tỉnh lược cần ngữ cảnh mới hiểu được). | xong | spec `2026-09-21-bao-bi-chan-tang-rag-design.md` §10 |
