...
| 19c | ✅ **ĐÓNG** (2026-09-22, 10 task TDD, nối tiếp 19b): vai bị chặn (`warehouse`) nay nhận **câu từ chối tất định nêu đúng phòng ban** thay vì câu trả lời tự tin lạc đề, trên CẢ hai tuyến `rag` và `mixed`. Cơ chế: `retrieve()` chạy thêm một truy vấn bóng KHÔNG lọc vai (chỉ khi vai đang bị hạn chế), lấy **top-3** ứng viên của bản bóng (`HIDDEN_TOP_K`, hằng cạnh `VIS_IDX`) — đổi từ luật hạng-1 ban đầu sau khi đo thật (xem dưới); nếu lớp bị giấu nằm trong đó thì `rag_node`/`gather_docs` trả `denied_message()` tất định (không LLM), `fuse_answer` nối câu đó vào cuối câu trả lời ERP trên tuyến mixed. **Số đo cuối:** cổng ÂM `thương mại 10 ca (lộ 0, không báo chặn 1 — đã biết được miễn), khác 99 ca (kém đi 0, từ chối oan 0)`; cổng DƯƠNG admin `0,963=0,963` không hồi quy; lat_p50 admin 523ms / kho 558ms (+35ms trên corpus thật ~3 900 chunk — KHÁC con số +4,2ms đo trên fixture 2 tài liệu ở Task 5, con số đó không dùng được cho corpus thật); probe sống PASS cả `rag` và `mixed` trên hạ tầng thật, đối chứng vai `sales` thấy đủ trích dẫn. **Giả thuyết ban đầu bị bác bởi số đo:** plan kỳ vọng luật hạng-1 bắt 10/10 câu thương mại; đo thật chỉ 5/10 (0 lộ, 0 từ chối oan) — bảng theo k (1→5: 5/10, 6/10, 9/10, 9/10, 9/10) cho thấy top-3 là điểm tốt nhất (9/10, vẫn 0/99 oan; k=5 mới sinh 1/99 oan). Chủ dự án chọn top-3 từ bảng đó — đây là đánh đổi sản phẩm, không phải quyết định kỹ thuật thuần. **Ba việc còn mở:** (1) trên tuyến mixed, model fusion TỰ VIẾT một đoạn lý do SAI (đổ lỗi "ERP không có dữ liệu") đứng TRƯỚC câu từ chối tất định đúng — máy không bắt được vì đoạn đó không chứa marker, chỉ thấy khi đọc nguyên văn bằng mắt; sửa cần A/B prompt fuse, ngoài phạm vi nhánh này; (2) một câu thương mại ("đóng gói hàng ra sao trước khi chuyển đi", tài liệu `sla.docx`) không thể bắt ở bất kỳ k nào (hạng > 5 trong bản bóng) — miễn qua `KNOWN_UNFLAGGED` có kiểm rữa staleness (cổng FAIL nếu ca này bắt đầu bị bắt hoặc câu hỏi biến mất khỏi bộ ca); (3) tuyến thật của câu mixed trong probe sống KHÔNG được xác nhận độc lập qua Langfuse/log — bằng chứng chỉ là câu trả lời có cả ERP thật lẫn câu từ chối, nhất quán với mixed nhưng chưa soi trace | xong | spec `2026-09-21-bao-bi-chan-tang-rag-design.md` §10 |
