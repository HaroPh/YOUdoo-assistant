...
| 19b | **RBAC tầng RAG — MỞ LẠI 2026-09-19, đang brainstorm → spec.** Đo lại: 18 tài liệu / 4 561 chunk, 100% `visibility='all'`; 8 nội bộ / 44 chunk như cũ + 1 tệp SID (660 chunk) nạp tay lúc test OCR. **Đã chốt:** (a) 4 tài liệu thương mại (`bang_gia`, `discount_policy`, `payment_policy`, `sla`) → `accounting`+`sales`+`admin`, `warehouse` bị chặn; 4 tài liệu quy trình giữ `all`; (b) corpus chung **chỉ chứa tri thức tổ chức** (`seed/`) — tài liệu người dùng đi endpoint trích xuất → kho riêng Open WebUI, **không bao giờ** vào `rag_documents`; (c) **SID GỠ khỏi corpus**, không gắn nhãn (đã gây ô nhiễm chéo: trả số SID cho câu hỏi NTC khi Ollama tắt). Lọc ở SQL trong `retrieve()` trên **cả 3 chân** dense/sparse/fold; fail-closed về `('all',)`. Tiền đề #28 ĐÃ XONG — nghiệm thu = `run_eval --set retrieval --model bge-m3 --baseline evals/baseline-bge-m3-retrieval.json` phải GATE PASS sau khi lọc + gỡ SID | **phiên 19b** (MCP/RAG) | spec sắp có ở `docs/superpowers/specs/2026-09-19-rbac-tang-rag-design.md`; nghiên cứu nền: `docs/odoo-mcp-server-nghien-cuu-2026-09-19.md` §5 mục 7 |
