...
| 25 | **Cổng eval dùng LLM vẫn chưa có lịch.** Hai cổng KHÔNG tốn hạn mức (`retrieval`, `multiturn` — `NO_LLM_SETS`) đã có lịch **hàng tuần** từ 2026-09-25: task `Youdoo-CongTruyXuat-HangTuan` (thứ Hai 10:00, `StartWhenAvailable`, chờ Docker tối đa 5 phút) chạy `scripts/chay-cong-truy-xuat.ps1`, mỗi lượt ghi một dòng vào `logs/jobs/cong-truy-xuat.log`; mã thoát 0 PASS · 1 trượt · 2 lỗi hạ tầng · 3 bỏ qua vì Docker chưa bật. Lịch cũ `ERP-AI-EvalGate` (trỏ `D:\Project`, thất bại mã 3) đã **vô hiệu hoá**, chưa xoá. CÒN MỞ: các bộ gọi LLM (`intent`, `confirm`, `planner`… ) vẫn chỉ chạy khi có người gõ lệnh | chưa ai | cần quyết: chạy định kỳ các bộ gọi LLM thì tốn hạn mức API mỗi lượt |
...
| kết luận | chứng cứ |
|---|---|
| **Open WebUI dùng reranker CỦA PROJECT cho tệp đính kèm** (đóng #26, 2026-09-25). Engine = External → backend `/v1/rerank` (bge-reranker-v2-m3 trên GPU, ~21 ms); `Top K = 10`, `Top K Reranker = 5`. Đo 11 câu trên 2 tệp thật: **10/11 với 5 chunk** — bằng mức nền cosine nhưng mức nền cần 10 chunk; hai câu khó (đáp án hạng 6 và 5 theo cosine) được reranker kéo lên hạng 3. Chọn 5 chứ không 3 vì hai câu đó nằm ĐÚNG hạng 3. **Hai bẫy của Open WebUI 0.11**: (1) mô tả trong UI SAI với mã — `Top K` quyết định CẢ số ứng viên lẫn số chunk cuối, `Top K Reranker` là trần đầu ra của reranker; cài `3/10` không có reranker cho **8/11**; (2) UI cho lưu Engine = External mà để TRỐNG *Reranking Model* — `get_rf` có `if reranking_model:` nên KHÔNG dựng reranker nào, âm thầm chấm cosine, đo ra **9/11** trong khi giao diện trông như đã bật. Backend tắt ⇒ Open WebUI giao nguyên danh sách chưa xếp (không mất đáp án, chỉ nhiễu hơn, một dòng cảnh báo trong log của nó). | `backend/tools/compare_attachment_rerank_result.txt`; chạy lại: `python -m tools.compare_attachment_retrieval --rerank-sweep` — hàng "như đang cấu hình" dựng reranker bằng CHÍNH `get_rf` của Open WebUI từ cấu hình sống, và từ chối ra số nếu reranker không trả điểm |
| **`RAG_RERANK_MODE=override` cho production** (chủ dự án chốt 2026-09-25, đóng hàng 37b). bge-reranker-v2-m3 xếp thuần theo điểm cross-encoder thay vì hoà 1:1 với hạng RRF. MRR cùng chiều trên ba bộ: TVPL +0,042, Zalo +0,048 (p<0,0001, pool tất định), nội bộ 109 ca +0,055 (có dấu). **Cái giá đã biết và chấp nhận**: mất lưới an toàn — đúng 2 câu hợp đồng lao động rơi từ hạng 3–4 xuống hạng 7 ở dạng có dấu (r@6 nội bộ 0,9633 → 0,9495), +1 câu giao hàng lên. Cổng `retrieval` nay NÉM khi baseline khác `rerank_mode` (trước đó so override với baseline blend thì PASS lặng lẽ); ba baseline đã chốt lại dưới override. **Backend phải khởi động lại** mới nhận biến mới. | `.env.example` (chú thích có số đo); `backend/evals/results/bench-ngoai-2026-09-24-tiebreak/README.md` |
