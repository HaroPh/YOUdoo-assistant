...
| 26 | **Open WebUI: cài đặt mới (2026-09-25) `top_k=3`, `top_k_reranker=10` CHƯA đạt mục tiêu đã đo.** Mô tả trong UI ("Top K Reranker = số kết quả gửi vào reranker") SAI với mã đang chạy (`open_webui/retrieval/utils.py`): BM25 lấy `top_k`, vector lấy `top_k`, reranker giữ `top_n = top_k_reranker`, rồi cắt về `min(top_k, top_k_reranker)`. Và `rag.reranking_model` **rỗng** — không có reranker thật, "rerank" chỉ là chấm cosine (xếp thuần dense). Với cài đặt mới: ≤6 ứng viên, model nhận **3** chunk chọn theo dense, chunk chỉ BM25 tìm ra vẫn bị loại. Mức đã đo 8/11 → 10/11 là khi model nhận **10** chunk (`top_k=10`, `top_k_reranker≥10`). Muốn "nhiều ứng viên → reranker → ít chunk tốt" thì phải cấu hình reranker thật trước, và điều đó CHƯA đo | chủ dự án | ghi chú thi hành `2026-08-31-tang-nap-tai-lieu-ghi-chu-thuc-thi.md`, mục hướng B bước 2 |
